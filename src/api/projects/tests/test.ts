...
const api : {client_id: string, client_secret: string, grant_type: string, code?:string|undefined, redirect_uri?:string|undefined}=  require('./api.json');
	...
	(async function () {
		...
		let ac = new ProjectsApi(base);
		//data = await ac.get_cursus_cursusId_projects(49);
		//data = await ac.get_me_projects();
		data = await ac.get_projects({"range[id]": "1,10"});
		//data = await ac.get_projects_id(8);
		//data = await ac.get_projects_projectId_projects(8);
		console.log(data)
