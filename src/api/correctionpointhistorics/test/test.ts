...
import { CorrectionPointHistoricsApi } from "../correctionpointhistorics.api";
...
const api : {client_id: string, client_secret: string, grant_type: string, code?:string|undefined, redirect_uri?:string|undefined}=  require('../../../tests/api.json');
	...
	(async function () {
		...
		const testapi = new CorrectionPointHistoricsApi(baseapi)
		data = await testapi.get_users_userId_correctionPointHistorics('eharuni')
